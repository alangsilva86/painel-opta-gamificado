...
export function LeversSection({
  ...
  const [paretoVisibility, setParetoVisibility] = useState({
    comissao: true,
    cumulativo: true,
  });
  ...
  const paretoData = bySeller
    ...
    .map((item, idx, arr) => {
      ...
      const cum = arr
        .slice(0, idx + 1)
        .reduce((acc, it) => acc + it.comissao, 0);
    ...
    ? (byProductOperation ?? []).flatMap(group => {
        ...
        return (group.operations ?? []).map((op: any) => {
          ...
          const matchesOperation =
            filterState.tipoOperacao.includes(tipoOperacao);
          const matches =
            (!anyProductFilter || matchesProduct) &&
            (!anyOperationFilter || matchesOperation);
            ...
            <BarChart data={paretoData}>
              ...
              <Tooltip
                content={({ payload }) => {
                  ...
                  return tooltipBox(item?.vendedor, [
                    {
                      label: "Comissão",
                      value: formatCurrency(item?.comissao ?? 0),
                      emphasis: true,
                    },
                    {
                      label: "Comissão média",
                      value: formatPercent(item?.takeRate ?? 0),
                    },
                    {
                      label: "Cumulativo",
                      value: `${(item?.cumulativo ?? 0).toFixed(1)}%`,
                    },
                  ]);
                }}
              />
              <Legend
                ...
                formatter={(value, entry) => {
                  ...
                  const isHidden =
                    key === "comissao"
                      ? !paretoVisibility.comissao
                      : key === "cumulativo"
                        ? !paretoVisibility.cumulativo
                        : false;
                  return (
                    <span
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart>
              ...
              <YAxis
                ...
                domain={[
                  0,
                  (dataMax: number) => (dataMax ? dataMax * 1.1 : 0.05),
                ]}
              />
