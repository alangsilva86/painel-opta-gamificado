...
export const contratos = mysqlTable(
  "contratos",
  {
    ...
    pctComissaoBonus: decimal("pct_comissao_bonus", {
      ...
      .default("0")
      .notNull(),
    vendedorId: varchar("vendedor_id", { length: 128 }).default("").notNull(),
    vendedorNome: varchar("vendedor_nome", { length: 255 })
      .default("Sem info")
      .notNull(),
    digitadorId: varchar("digitador_id", { length: 128 }).default("").notNull(),
    digitadorNome: varchar("digitador_nome", { length: 255 })
      .default("Sem info")
      .notNull(),
    produtoId: varchar("produto_id", { length: 128 }).default("").notNull(),
    produto: varchar("produto", { length: 255 }).default("Sem info").notNull(),
    tipoOperacaoId: varchar("tipo_operacao_id", { length: 128 })
      .default("")
      .notNull(),
    tipoOperacao: varchar("tipo_operacao", { length: 255 })
      .default("Sem info")
      .notNull(),
    agenteLookupId: varchar("agente_lookup_id", { length: 128 })
      .default("")
      .notNull(),
  ...
  table => ({
    idxDataPagamento: index("idx_contratos_data_pagamento").on(
      table.dataPagamento
    ),
    idxVendedor: index("idx_contratos_vendedor_nome").on(table.vendedorNome),
    idxVendedorId: index("idx_contratos_vendedor_id").on(table.vendedorId),
    ...
    idxProdutoId: index("idx_contratos_produto_id").on(table.produtoId),
    idxTipoOperacao: index("idx_contratos_tipo_operacao").on(
      table.tipoOperacao
    ),
    idxAgente: index("idx_contratos_agente").on(table.agenteId),
    idxDataEtapa: index("idx_contratos_data_etapa").on(
      table.dataPagamento,
      table.etapaPipeline
    ),
    idxDataVendedor: index("idx_contratos_data_vendedor").on(
      table.dataPagamento,
      table.vendedorNome
    ),
  })
